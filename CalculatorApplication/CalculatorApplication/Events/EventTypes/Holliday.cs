...
namespace CalculatorApplication.Events.EventTypes
{
    public class Holliday : BaseEvent
    {
        ...
        public string pathSuffix = "Holliday.txt";
        ...
        public void NewEvent()
        {
            try
            {
                ...
                Console.WriteLine("what is the holliday?");
                var naming = Console.ReadLine();
                HollidayName = naming.ToString();

                EndDate = endMonth + endDay;

                Console.WriteLine($"is this correct? {EventName}, {ID}, {StartDate}, {EndDate}, {EventDescription}, {HollidayName}");
        ...
        public static void SeeEventsForDate(string filePath)
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        var deserializedEvents = JsonConvert.DeserializeObject<Holliday>(item);
                        Console.WriteLine($"event : {deserializedEvents.EventName}, {deserializedEvents.EventDescription}, {deserializedEvents.ID}, starts: {deserializedEvents.StartDate}, ends: {deserializedEvents.EndDate}, holliday name : {deserializedEvents.HollidayName}");
        ...
        public static void DeleteEvent(string filePath)
        {
            try
            {
                ...
                else
                {
                    ...
                    string fileText = File.ReadAllText(filePath);
                    var splittedDates = fileText.Split("---");
                    List<Holliday> EventList = new List<Holliday>();

                    foreach (var item in splittedDates)
                    {
                        var deserializedEvents = JsonConvert.DeserializeObject<Holliday>(item);
                        EventList.Add(deserializedEvents);
                        Console.WriteLine($"event : {deserializedEvents.EventName}, {deserializedEvents.EventDescription}, {deserializedEvents.ID}, starts: {deserializedEvents.StartDate}, ends: {deserializedEvents.EndDate}, {deserializedEvents.EventDescription}");
